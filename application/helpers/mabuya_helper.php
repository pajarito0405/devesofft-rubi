...
if (!function_exists('get_image_array_extension')) {

    function get_image_array_extension($img_type)
    { 
      for($i=0; $i<=sizeof($img_type); $i++){
        if (empty($img_type[$i]))
            return false;
...
if (!function_exists('save_image_from_post')) {
    function save_image_from_post($image_control, $folder, $image_name, $width = 0, $height = 0, $id_bus)
    { 
        $response = [];
      //var_dump($image_control['error']); die();
        /*if ($image_control['error'] != UPLOAD_ERR_OK) {
            return [FALSE, get_upload_error_message($image_control['error'])];
        }*/
        $img_ext = get_image_array_extension($image_control["type"]);

        if ($img_ext) {
            if (!is_dir($folder)) {
                if (!mkdir($folder, DIR_READ_MODE, TRUE)) {
                    $response[0] =  [FALSE, "Folder $folder could not be created"];
                }
            }
         
            foreach($image_control["tmp_name"] as $k => $f){
    $image_path[$k] = $folder . "/" . $_SESSION['id_cooperativa'] ."-". $id_bus . "-" .  $image_name[$k] . $img_ext;
    $image_path_tmp[$k] = $folder . "/" . $_SESSION['id_cooperativa'] ."-". $id_bus . "-" . $image_name[$k] . "temp" . $img_ext;

               if ($width != 0 && $height != 0) {
                
                  $valid_copy = copy($f, $image_path_tmp[$k]);
                  if ($valid_copy == FALSE) {
                      $response[$k] =  array(FALSE, "The file can't be overwritten");
                  }
                  
                  $result = image_resize($image_path_tmp[$k], $image_path[$k], $width, $height);

                  if (is_bool($result)) {
                      unlink($image_path_tmp[$k]);
                      $response[$k] =  array(TRUE, $image_path[$k]);
                  }
               } else {

                  $valid_copy = copy($f, $image_path[$k]);
                  if ($valid_copy == FALSE) {
                      $response[$k] =  array(FALSE, "The file can't be overwritten");
                  }
                  $response[$k] =  array(TRUE, $image_path[$k]);
            ...
            $file_name = $image_control['name'];
            $response[0] =  array(FALSE, "The file is not allowed because it's extension");
        }
...
if (!function_exists('save_image_from_post_2')) {
    function save_image_from_post_2($image_control, $folder, $image_name, $width = 0, $height = 0)
    {
