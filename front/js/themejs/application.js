...
gfx(window).load(function () {
    var pageLoading = gfx("#page-preloader");
    setTimeout(function () {
        pageLoading.fadeOut();
    }, 1200);
});
    ...
    gfx(window).scroll(function (e) {
        ...
        if (gfx(this).scrollTop() > 400 && $el.css('position') != 'fixed') {
            gfx('.header-bottom').css({
                ...
                'top': '0px',
                'opacity': '0'
            }).animate({opacity: 1}, 600).addClass('visible');
            ;
        }
...
gfx('#list').click(function () {
    gfx('#products .item').addClass('list-group-item');
});
gfx('#grid').click(function () {
    gfx('#products .item').removeClass('list-group-item');
});
